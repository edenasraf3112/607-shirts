...
async function queryOrders(filter: string, collectionId?: string | null, dateFrom?: string | null, dateTo?: string | null) {
  const db = getServiceClient()

  // Base query — always use select('*') like the working orders page
  ...
  // "Paid" filter — prefer paid_at IS NOT NULL, fall back to status
  if (filter === 'paid') {
    try {
...
function aggregateOrders(orders: any[]): ProductionRow[] {
...
export async function GET(req: Request) {
  ...
  try {
    const orders = await queryOrders(filter, collectionId, dateFrom, dateTo)
